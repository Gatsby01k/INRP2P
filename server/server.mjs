...
async function routeApi(req, res, url) {
  ...
  if (req.method === 'POST' && url.pathname === '/api/round/start') {
    try {
      ...
      const round = {
        id, userId: session.uid, bet, seed: seed.toString('hex'), commitment, bustAt, overdriveAt,
        impactCount: 0, currentMultiplier: 1, status: 'active', createdAt: Date.now(), settledAt: null, payout: 0
      };
      rounds.set(id, round);
  ...
  if (req.method === 'POST' && impactMatch) {
    ...
    return json(res, 200, {
      status: 'survived', impactCount: round.impactCount, multiplier: round.currentMultiplier,
      overdrive: round.overdriveAt === round.impactCount, damageStage: Math.min(7, round.impactCount)
    });
  }
  ...
  if (req.method === 'POST' && cashoutMatch) {
    ...
    if (!round || round.userId !== session.uid) return json(res, 404, { error: 'ROUND_NOT_FOUND' });
    if (round.status !== 'active') return json(res, 409, { error: 'ROUND_NOT_ACTIVE', status: round.status });
    // This state transition is synchronous in this process. Production must execute it in one DB transaction.
    round.status = 'cashed_out'; round.settledAt = Date.now();
    round.payout = Math.round(round.bet * round.currentMultiplier);
...
async function serveStatic(req, res, url) {
  ...
  try {
    ...
    const immutable = /\/assets\//.test(pathname);
    res.writeHead(200, {
