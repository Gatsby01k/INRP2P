...
export function TrainingReserveWorkflow({
  ...
  return (
    <>
      <PageHeader
        title={activated ? "Training reserve" : "Activate your training desk"}
        sub="Practice the complete reserve workflow without a wallet, blockchain transaction or real funds."
        actions={<span className="chip border-sky-200 bg-sky-50 text-sky-700"><span className="h-1.5 w-1.5 rounded-full bg-sky-500" />Simulation</span>}
      />
      ...
      <section className="mb-5 overflow-hidden rounded-2xl border border-[#07152e]/10 bg-[#07152e] text-white shadow-card">
        <div className="grid gap-6 px-5 py-6 sm:px-7 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-center">
          <div>
            <p className="text-[9px] font-semibold uppercase tracking-[0.18em] text-sky-300">Isolated training ledger</p>
            ...
            <p className="mt-2 max-w-2xl text-xs leading-6 text-white/55">
              This workflow records the same instruction, submission and operator-review states used by the product. The destination wallet is intentionally disabled.
            </p>
          </div>
        ...
        <div className="grid gap-px border-t border-white/10 bg-white/10 sm:grid-cols-3">
          {[
            ["01", "Create instruction", "Record the exact simulated amount."],
            ["02", "Submit transfer", "Generate a non-financial training reference."],
            ["03", "Operator activates", "Studio enables the training order limit."],
          ].map(([step, title, body]) => (
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_330px]">
            ...
            {!current ? (
              <form action={createTrainingReserveInstruction}>
                ...
                <div className="rounded-xl border border-dashed border-black/[0.12] bg-black/[0.015] p-5">
                  <p className="text-xs font-semibold text-slate-900">Ready to create a training instruction</p>
                  <p className="mt-1 text-[10px] leading-5 text-slate-500">The instruction will use {selectedLevel.activationReserveUsdt.toLocaleString("en-US")} simulated USDT and cannot receive funds.</p>
                  <SubmitButton className="btn btn-gold mt-4" pendingLabel="Creating…">Create training instruction →</SubmitButton>
                </div>
              </form>
            ) : (
              <div>
                <div className="flex flex-col gap-4 rounded-xl border border-black/[0.08] bg-[#fbfaf8] p-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="mt-2 text-xl font-semibold tabular-nums text-slate-900">{usdt(current.actualAmount ?? current.amount)} <span className="text-[10px] text-slate-400">simulated USDT</span></p>
                    <p className="mt-1 text-[9px] text-slate-400">TRAINING_SIMULATOR · no destination wallet</p>
                  </div>
                  {current.status === "AWAITING_PAYMENT" ? (
                    <form action={submitTrainingReserveInstruction}>
                      <input type="hidden" name="depositId" value={current.id} />
                      <SubmitButton className="btn btn-gold btn-sm" pendingLabel="Submitting…">Simulate transfer sent →</SubmitButton>
                    </form>
                  ) : current.status === "CONFIRMING" ? (
                    <div className="max-w-[230px] rounded-lg border border-sky-200 bg-sky-50 px-3 py-2 text-[10px] leading-4 text-sky-700">Submission recorded. Move the scenario to “Order desk ready” from Training Studio.</div>
                  ) : (
                    <Link href="/partner/processing" className="btn btn-gold btn-sm">Open training orders →</Link>
                  )}
                </div>
        ...
        <aside className="card h-fit p-5">
          <p className="text-[9px] font-semibold uppercase tracking-[0.14em] text-sky-700">Training boundary</p>
          <ul className="mt-4 space-y-3 text-[10px] leading-5 text-slate-500">
            ...
            <li><strong className="text-slate-800">No live activation:</strong> only Training Studio can enable this desk.</li>
          </ul>
        </aside>
      </div>
    </>
