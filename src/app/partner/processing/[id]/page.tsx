...
export default async function PartnerProcessingOrderPage({
  params,
  searchParams,
}: {
  ...
  const order = await db.processingOrder.findFirst({
    where: { id, partnerId: user.partner.id },
    include: { company: true, partner: true, rail: true, settlement: true, events: true },
  });
  if (!order) notFound();
  return <OrderDetailView order={order} viewer="partner" notice={query.notice} error={query.error} />;
}
